...
# Bind the cluster number to the original data used for clustering, so that we can see what cluster each penguin is assigned to (SUPER USEFUL SYNTAX TO ASSOCIATE THINGS FROM DIFF DFs TOGETHER!!!)
...
penguins_cl %>% 
  count(species, cluster_no) %>% # spec w/in each clust
...
# Update to add rownames (country name) from wb_ghg_20 (interesting way to have your data listed)
...
Great, now we have a simplified, scaled version of the numeric variables, with rownames containing the country name. 
